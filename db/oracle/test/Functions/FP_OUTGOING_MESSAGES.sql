DROP FUNCTION FP_OUTGOING_MESSAGES;

CREATE OR REPLACE FUNCTION FP_OUTGOING_MESSAGES
(
  LOCK_ID NVARCHAR2,
  FROM_DATE TIMESTAMP,
  TILL_DATE TIMESTAMP 
...
BEGIN
  ...
  /*FOR Inc IN (SELECT SPECTRE_UFS_ID
                 FROM CLIENTS
                WHERE CLIENT_ID=FP_CLIENT_MONEY.CLIENT_ID) LOOP
    Investor:=Inc.SPECTRE_UFS_ID;
    EXIT;    
  END LOOP;
  ...
  IF (Investor IS NOT NULL) THEN
    ...
    S1:='SELECT CURRENCY_NAME,
                ...
                NDFL,
                CF_BALANCE
           FROM UFS_CLIENT_MONEY('||F||','||T||','||Investor||')';
