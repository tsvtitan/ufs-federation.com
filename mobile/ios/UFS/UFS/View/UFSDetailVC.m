...
- (void)viewDidLoad
{
    ...
    [keywordButtons setListDelegate:self];
    ...
    [subscribeButton setTitleColor:[UIColor lightGrayColor] forState:UIControlStateDisabled];
...
- (NSArray *)getKeywords {
    ...
    NSEntityDescription *entity = [NSEntityDescription entityForName:@"KeywordDB" inManagedObjectContext:CoreDataManager.shared.managedObjectContext];
    
    [request setEntity:entity];
    NSSortDescriptor *desc = [[NSSortDescriptor alloc] initWithKey:@"identifier" ascending:YES];
    NSArray *descs = [[NSArray alloc] initWithObjects:desc,nil];
    
    [request setPredicate:nil];
    [request setSortDescriptors:descs];
    ...
    if (![results performFetch:&error]) {
        ...
        NSLog(@"getKeywords error: %@, %@", error, [error userInfo]);
        
    } else {
        if (results.fetchedObjects.count>0) {
            
            keywords = [[NSMutableArray alloc] init];
            for (int i=0; i<results.fetchedObjects.count; i++) {
                KeywordDB *kwd = (KeywordDB *)[results.fetchedObjects objectAtIndex:i];
                if (kwd!=nil) {
                    [keywords addObject:kwd.keyword];
                }
            }
        }
    }
...
- (BOOL)keywordExists:(NSArray*)keywords keyword:(NSString *)keyword {

    BOOL exists = false;
    if (keywords!=nil) {
        
        NSPredicate *predicate = [NSPredicate predicateWithFormat:@"SELF == %@",keyword];
        NSArray *filteredArray = [keywords filteredArrayUsingPredicate:predicate];
        id firstFoundObject =  filteredArray.count > 0 ? filteredArray.firstObject : nil;
        exists = firstFoundObject!=nil;
        
        /*for (int i=0; i<keywords.count; i++) {
            NSString *kwd = (NSString *)[keywords objectAtIndex:i];
            exists = [kwd isEqualToString:keyword];
            if (exists) {
                NSLog(@"keyword = %@",kwd);
                break;
            }
...
- (void)webViewDidFinishLoad:(UIWebView *)webView
{
    ...
    if ([_newsObj.keywords allObjects].count) {
        ...
        if (kwds.count>0) {
            ...
            for (int i=0; i<keywordButtons.subviews.count;i++) {
                UICheckButtonView *view = (UICheckButtonView*)[keywordButtons.subviews objectAtIndex:i];
                BOOL exists = [self keywordExists:keywords keyword:view.button.titleLabel.text];
                view.button.isChecked = !exists;
                [view.button setEnabled:!exists];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    /* tsv */
    newsDetail.titles = self.titles;
    newsDetail.title = db.title;
    /* tsv */
