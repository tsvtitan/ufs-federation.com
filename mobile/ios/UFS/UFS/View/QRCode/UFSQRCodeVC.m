...
- (void) showPromotion: (NSDictionary*) dictionary
{
    ...
    if (registered) {
        UFSPromotionVC *promotion = [[UFSPromotionVC alloc] initWithPromotion:dictionary titleParent:self.titleText];
        [self.navigationController pushViewController:promotion animated:YES];
        [promotion release];
    } else {
        UFSRegistrationVC *registration = [[UFSRegistrationVC alloc] initWithPromotion:dictionary titleParent:self.titleText];
        [self.navigationController pushViewController:registration animated:YES];
        [registration release];
    }
}

-(void) syncAfterQRCode:(NSNotification *)notify {
...
{
    [self performSelectorOnMainThread:@selector(syncAfterQRCode:) withObject:notify waitUntilDone:NO];
}
