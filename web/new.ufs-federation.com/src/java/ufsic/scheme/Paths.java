...
public class Paths extends SchemeTable {
  ...
  private Path getCertainPath(String path) {
    ...
    if (isNotNull(scheme.getLang())) {
      ...
      Filter f1 = new Filter().Add(Path).Equal(path.toLowerCase()).Or(PathId,pathId);
      Filter f2 = new Filter().Add(Locked).IsNull();
      Filter f = new GroupFilter(f1).And(f2);

      DataSet ds = provider.select(getViewName(),f,new Orders(Priority,LangId));
      if (isNotNull(ds)) {
  ...
  private boolean isPathRestricted(Class cls) {
  ...
  public Path queryPath(String path) {
    ...
    if (isNull(ret)) {
      ...
      if (isNotNull(ret)) {
        Class cls = ret.getHandlerClass();
        if (isPathRestricted(cls)) {
          ret = null;
        }
      }
    }
