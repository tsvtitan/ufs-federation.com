...
public class PatternMessage extends Message {
  ...
  protected boolean send(Value senderId, String senderName, String senderContact, 
                         Value recipientId, String recipientName, String recipientContact,
                         String subject, Timestamp begin, Timestamp end,
                         Integer priority, Value channelId, Value patternId) {
    ...
    if (isNotNull(scheme)) {
      ...
      if (isNotNull(body)) {
        Value id = scheme.getProvider().getUniqueId();
        if (id.isNotNull()) {
          ...
          if (insert()) {
            ...
            Attachments ats = super.getAttachments(false);
            if (isNotNull(ats) && !ats.isEmpty()) {

              for (Record r: ats) {
                Attachment a = (Attachment)r;
                if (a.getAttachmentId().isNull()) {
                  a.setAttachmentId(scheme.getProvider().getUniqueId());
                }
                flag = flag && a.insert();
              }
            }
  ...
  protected boolean send(String senderName, String senderContact, String recipientName, String recipientContact,
                         String subject, Timestamp begin, Timestamp end, Integer priority) {
    return send(null,senderName,senderContact,null,recipientName,recipientContact,subject,begin,end,priority,null,null);
  }
