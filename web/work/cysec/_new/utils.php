...
function collect_trans ($LOG) {
	...
	if ($fansy_db) {
		...
		if ($cysec_db) {
			...
			$reestr = $fansy_db->getRecords(sprintf('SELECT * FROM PR_DEALS_BY_DATE(%s)',"'".$dt."'"));
			if (is_array($reestr)) {
				...
				foreach ($reestr as $r) {
					...
					$rec = $cysec_db->getRecord(sprintf('SELECT count(*) as cnt '.
							 							'FROM trans '.
														'WHERE TransactionReferenceNumber=%s',"'".$r['TRANSACTION_REFERENCE_NUMBER']."'"));
...
function send_out($LOG, &$OUT) {
	...
	if ($cysec_db) {
		...
		if (is_array($out)) {
			...
			$LOG->writeInfo(sprintf('Out has selected (id => %d). Connecting to CySec ftp ...',$out['out_id']));
												...
												if (isset($NoErrors) && ($NoErrors->length>0)) {
													...
													if ($NoErrors->nodeValue=='OK') {
														...
														$tran_ids = $cysec_db->getRecords(sprintf('SELECT tos.tran_id '.
																                                  ...
																                                  'WHERE t.processed is null and tos.out_id=%d',$out['out_id']));
...
function send_report_by_trans($LOG, $NAME, $TRANS, $ERROR, $FILES) {
	...
	foreach ($emails as $m) {
		...
		$headers.='From: CySEC <quik@ufs-federation.com>';
		...
		if (is_array($TRANS)) {
			foreach ($TRANS as $t) {
				...
				$message.=sprintf('<span style="color: gray">Instrument:</span> %s (%s)<br>'."\n",$t['instrument_name'],$t['InstrumentIdentification']);
	...
	if ($cysec_db) {
		...
		try {
			...
			$trans = $cysec_db->getRecords(sprintf('SELECT * '.
					                               ...
					                               'WHERE tos.out_id=%s '.
					                               'ORDER BY TradingDay, TradingTime, tran_type',$OUT['out_id']));
			if (is_array($trans)) {
		...
		try {
			...
			foreach ($INS as $in) {
				...
				if (is_array($out)) {
					...
					$LOG->writeInfo('Out has an error');
					
				    $trans = $cysec_db->getRecords(sprintf('SELECT * '.
						                                   'FROM trans t '.
								                           'LEFT JOIN tran_outs tos on tos.tran_id=t.tran_id '.
								                           'WHERE tos.out_id=%s '.
								                           'ORDER BY TradingDay, TradingTime, tran_type',$in['out_id']));
					if (is_array($trans)) {
						$LOG->writeInfo(sprintf('There are %d tran(s). Sending trans and file ...',sizeof($trans)));
					} else {
				    ...
				    $sent = send_report_by_trans($LOG,$s,$trans,$out['error'],$files);
