...
set_include_path(':/install/php/work/mailing:'.get_include_path());

require_once 'consts.php';

set_include_path(':/install/php/work/libs:'.get_include_path());
...
$log = new Log (LOG_MAILING,DEBUG_LOG);
if ($log) {
  ...
  try {
    ...
    if ($db) {
      ...
      $db->query('DELETE FROM mailing '.
                  'WHERE created<=ADDDATE(CURRENT_TIMESTAMP, INTERVAL -7 DAY);');
      
      $db->query('DELETE FROM mailing_patterns '.
                  'WHERE mailing_pattern_id NOT IN (SELECT DISTINCT(mailing_pattern_id) '.
                                                     'FROM mailing);');
        ...
        if (is_array($mailing) && sizeOf($mailing)>0) {
          ...
          if ($r) {
            ...
            foreach($emails as $e) {
              ...
              if (!is_null($e->sent)) {
                $log->writeInfo(sprintf('Mailing %d has sent successfully at %s...',$e->mailing_id,$e->sent));
              } else {
                $log->writeError(sprintf('Mailing %d has not sent. Error is %s ...',$e->mailing_id,$e->result));
              }
              
              $r = $db->updateRecord('mailing',$data,$where);
              if ($r) {
                $log->writeInfo(sprintf('Mailing %d has updated successfully ...',$e->mailing_id));
              } else {
                $log->writeError(sprintf('Mailing %d has not updated. Error is %s ...',$e->mailing_id,$r));
              }
            }
          } else {
              ...
              if ($flag) {
                ...
                foreach ($mailing as $m) {
                  ...
                  if ($r) {
                    ...
                    $log->writeInfo(sprintf('Mailing %d has %s successfully at %s...',
                                            $m['mailing_id'],($method==0)?'transferred':'sent',$d));
                  } else {
                    $log->writeError(sprintf('Mailing %d has not %s. Error is %s ...',
                                             $m['mailing_id'],($method==0)?'transferred':'sent',$r));
                  }
                  
                  $r = $db->updateRecord('mailing',$data,$where);
                  if ($r) {
                    $counter = $counter + $increment;
                    $log->writeInfo(sprintf('Mailing %d has updated successfully ...',$m['mailing_id']));
                  } else {
                    $log->writeError(sprintf('Mailing %d has not updated. Error is %s ...',$m['mailing_id'],$r));
                  } 
                }
                ...
                if ($r) { 
                  ...
                  foreach ($emails as $e) {
                    ...
                    if (!is_null($date)) {
                      $log->writeInfo(sprintf('Mailing %d has %s successfully at %s...',
                                              $e->mailing_id,($method==0)?'transferred':'sent',$date));
                    } else {
                      $log->writeError(sprintf('Mailing %d has not %s. Error is %s ...',
                                               $e->mailing_id,($method==0)?'transferred':'sent',$e->result));
                    }
