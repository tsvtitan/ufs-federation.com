...
// by TSV
function update_operator($operatorid, $login, $email, $phone, $occupation, $sex, $jabber, $password, $localename, $commonname, $notify)
{
	global $mysqlprefix;
	$link = connect();
	$query = sprintf(
    ...
    // by TSV      
		"update ${mysqlprefix}chatoperator set vclogin = '%s',%s vclocalename = '%s', vccommonname = '%s'" .
		", vcemail = '%s', vcphone = '%s', vcoccupation = '%s', vcsex = '%s', vcjabbername= '%s', inotify = %s" .
		" where operatorid = %s",
    ...
    mysql_real_escape_string($occupation, $link),
    mysql_real_escape_string($sex, $link),
...
// by TSV
function create_operator_($login, $email, $phone, $occupation, $sex, $jabber, $password, $localename, $commonname, $notify, $avatar, $link)
{
	global $mysqlprefix;
	$query = sprintf(
    ...
    // by TSV
    "insert into ${mysqlprefix}chatoperator (vclogin,vcpassword,vclocalename,vccommonname,vcavatar,vcemail,vcphone,vcoccupation,vcsex,vcjabbername,inotify) "
...
// by TSV
function create_operator($login, $email, $phone, $occupation, $sex, $jabber, $password, $localename, $commonname, $notify, $avatar)
{
	$link = connect();
	$newop = create_operator_($login, $email, $phone, $occupation, $sex, $jabber, $password, $localename, $commonname, $notify, $avatar, $link);
	mysql_close($link);
	return $newop;
}
...
/* tsv */
function changeDtmLastVisited($arr) {
  
  global $mysqlprefix, $settings;
  
  $link = connect();
  loadsettings_($link);
  $sql = sprintf("update ${mysqlprefix}chatoperator set dtmlastvisited = date_add(dtmlastvisited,interval %s second) where operatorid = %s", 
                 '-'.$settings['online_timeout'], intval($arr['operatorid']));
...
function logout_operator()
{
  ...
  /* tsv */changeDtmLastVisited($_SESSION["${mysqlprefix}operator"]);
