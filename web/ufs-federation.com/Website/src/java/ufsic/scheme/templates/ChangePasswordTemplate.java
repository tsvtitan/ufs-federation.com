  ...
  public void process(IVarContext context) {
    ...
    if(isNull(account))
    {
      ...
      }else
      {
        ...
        if(errorCode != null)
        {
          context.setLocalVar("errorCode", errorCode.toString());
        }else
        {
    ...
    if(errorCode == null)
    {
      switch(changePassword.getStepFlow())
      {
          ...
          }else
          {
            ...
            if(phone.isNotNull())
            {
              ...
              */
              ...
              context.setLocalVar("hasPhone", true);
            ...
            }else
            {
              ...
              if(email.isNotNull())
              {
                ...
                //m.setBody("Для завершения операции пройдите по ссылке https://" + getScheme().getURI().getHost() + ":" + getScheme().getURI().getPort() + 
                //        getScheme().getPath().getRootPath("/change-password-confirmation?code=") + confirmCode);
                m.setBody("Для завершения операции пройдите по ссылке " + getScheme().getPath().getAddress() + 
                        getScheme().getPath().getRootPath("/change-password-confirmation?code=") + confirmCode);

                context.setLocalVar("hasEmail", true);
        ...
        case 2:
          ...
          if(errorCode != null)
          {
            context.setLocalVar("confirmCode", changePassword.getConfirmationCode());
      ...
      context.setLocalVar("step", changePassword.getStepFlow());
