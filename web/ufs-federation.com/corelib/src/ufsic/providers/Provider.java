...
abstract public class Provider extends CoreObject implements IProvider {
  ...
  public Provider(Logger logger, String jndiName) {
    
    this(null,logger,jndiName);
  }
  ...
  public Dataset querySelect(String sql, ArrayList<Object> args, IProviderSelector selector) {
    ...
    Dataset ret = null;
    Object r = querySelect(sql,true,args,selector);
    if (isNotNull(r)) {
      ...
      try {
        ...
        try {
          ...
          if (isNotNull(stmt)) {
            ...
            List<Direction> drIn = Arrays.asList(new Direction[]{Direction.IN,Direction.IN_OUT});
            List<Direction> drOut = Arrays.asList(new Direction[]{Direction.IN_OUT,Direction.OUT});
            
            if (isNotNull(params)) {
              
              for (Param p: params) {
                String n = quoteName(p.getName());
                if (drIn.contains(p.getDirection())) {
                  stmt.setObject(n,p.getValue().getObject());
                }
                if (drOut.contains(p.getDirection())) {
      ...
      if (!sql.trim().equals("")) {  
        ...
        /*if (isNotNull(params)) {
          ...
          for (Param pp: procParams) {
            ...
            boolean need = drIn.contains(pp.getDirection());
            ...
            if (need) {
              pp.setValue(v);
              if (isNotNull(sqlType)) {
                pp.setSqlType(sqlType);
              }
            }
          }
        }*/
  ...
  public Record getFirstRecord(Dataset<Record> dataset) {
  ...
  public Record first(String name, Filter filter, Orders orders) {

    return first(name,null,filter,orders,null);
  }
  ...
  public Record first(String name) {
    ...
    return first(name,null,(Filter)null);
  }
  
  public Value getFirstValue(Record record) {
  ...
  public Value firstValue(String name, Filter filter) {
    
    Record r = first(name,filter);
    return getFirstValue(r);
  }
  
  public Value firstValue(String name) {
  ...
  abstract public Value getUniqueId();
  
  abstract public Value getNow();
