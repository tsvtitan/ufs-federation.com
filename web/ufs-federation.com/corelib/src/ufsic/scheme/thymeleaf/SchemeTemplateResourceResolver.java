...
public class SchemeTemplateResourceResolver extends SchemeObject implements IResourceResolver {
  ...
  public InputStream getResourceAsStream(TemplateProcessingParameters templateProcessingParameters, String resourceName) {
    ...
    InputStream ret;
    Template t = scheme.getTemplates().findFirst(Templates.TemplateId,resourceName);
    if (isNotNull(t)) {
      Value locked = t.getLocked();
      if (locked.isNull()) {
      ...
      Record r = provider.first(scheme.getTemplates().getViewName(),fn,new Filter(Templates.TemplateId,resourceName));
      if (isNotNull(r)) {
        Value locked = r.getValue(Templates.Locked);
        if (locked.isNull()) {
          t = scheme.getTemplates().newTemplate(r,r.getValue(Templates.TemplateType));
          t.process(new TemplateContext(resourceName,templateProcessingParameters));
          ret = t.getStream(Templates.Html);
        } else {
          ret = new ByteArrayInputStream("".getBytes());
