...
public class FileHandler extends Handler {
  ...
  private Location getLocation(File file) {
    ...
    if (location.isNotNull()) {
      ...
      p = p.replace(Utils.getUrlSeparator(),Utils.getPathSeparator());
      sb.append(p);
  ...
  public boolean process(Comm comm) {
    ...
    if (isNotNull(scheme)) {
      ...
      if (isNotNull(r)) {
        ...
        Location loc = new Location(path.getRestOfRootPath(path.getPath().asString()),scheme.getDefaultDir());
        if (loc.exists()) {
          java.io.File f = loc.getFile();
