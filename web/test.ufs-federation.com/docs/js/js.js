...
$(document).ready(function() {
	/* $('.more-qst').click(function(){
		$(this).parent().prev('.more-qst-list').slideToggle();
		$(this).hide();
		return false;
	}); */
	$('.more-qst').click(function(){
		mq = $(this).parent().prev('.more-qst-list');
		if(mq.css('display') == 'none') {
			$(mq).slideDown(600);
			$(this).find('span').text('Свернуть');
		} else if (mq.css('display') == 'block') {
			$(mq).slideUp(400);
			$(this).find('span').text('Все вопросы');
		}
		return false;
	});
});