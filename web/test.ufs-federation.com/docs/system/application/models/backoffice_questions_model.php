<?php
class backoffice_questions_model extends Model{

	function backoffice_questions_model()
	{
		parent::Model();
    ...
    {                        
        if(isset($_REQUEST['submit'])){
            $ret=$this->global_model->adjustment_of_request_array($_REQUEST);
            $ret['answer'] = $_REQUEST['text']['answer'];
      ...
      $content['data']='';
        ...
        if($sql_r->num_rows()>0){	
            ...
            $vals='`t`.`id`, `t`.`name`, `t`.`timestamp`, `c`.`id` as `com_id`, `c`.`name` as `user`, 
                   ...
                   case when c.email is null or (trim(c.email)="") then c.name else concat(c.name," (",c.email,")") end as name_email';
