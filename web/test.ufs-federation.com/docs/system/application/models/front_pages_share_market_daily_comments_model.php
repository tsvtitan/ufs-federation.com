<?php
class front_pages_share_market_daily_comments_model extends Model{

	function front_pages_share_market_daily_comments_model()
	{
		parent::Model();
	}
    ...
    function last($limit=3)
    {
      $ret=array();
            ...
            $sql=$this->db->query('select `url`,`name`,`short_content`,`timestamp` 
                                   from `share_market_daily_comments` 
                ...
                if($sql->num_rows()>0){
                    $res=$sql->result();
                        ...
                        /*********************************/
                         $res=$this->global_model->adjustment_of_results($res,'share_market_daily_comments');
                         $res=$this->global_model->date_format($res,'j #_n_# Y');
                        /*********************************/
                         
                    $page_url='pages/'.$this->global_model->pages_url('share_market_daily_comments');
    ...
    function view($ret)
    {
        ...
        $data['page_url']=$ret->page_url='pages/'.$this->global_model->pages_url('share_market_daily_comments');
        ...
        if(!empty($url) and $url!='all'){
          ...
          $sql_r=$this->db->query(self::view_sql_str($vals,$where,$limit));
            ...
            if($sql_r->num_rows()>0){	
                ...
                $show=5;
                $rows=$res_r->Rows;
                $ret_page=((int)$this->uri->segment(8)==0)?1:(int)$this->uri->segment(8);
                $param['url']=$data['page_url'].'/all';
           ...
           if(isset($sql)){
                if($sql->num_rows()>0){
                    $res=$sql->$res_func();
                        ...
                        $data[$res_data]=$res;
                        $ret->content=$this->load->view('view_pages_share_market_daily_comments',$data,true);  
                }
           }
