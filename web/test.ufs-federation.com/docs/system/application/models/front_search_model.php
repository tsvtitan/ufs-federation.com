...
class front_search_model extends Model{
  ...
  private function get_real_text($text) {
  
    $ret =  $text;
    
    $res = $this->maindb_model->select_table('search_keys','lang="'.$this->site_lang.'"','`priority`');
    if (is_array($res) && sizeOf($res)>0) {
      ...
      foreach ($res as $r) {
        ...
        if (@preg_match($key,$text,$matches)) {
          ...
          $match = (sizeOf($matches)>1)?$matches[1]:'';
          $s = (trim($r->text)!='')?$r->text:$match;
          if (trim($s)!='') {
            $ret = $s;
            break;
          }
  ...
  private function mySortDate($a,$b) {
    ...
    {  
      ...
      $search_word     = '';
    	$kword           = isset($_REQUEST['text'])?$_REQUEST['text']:'';
    	...
    	if (isset($option)) {
    	  if (empty($option)) { 	
            $option = SPH_MATCH_ANY;
    	  	...
    	  	if (($option!=SPH_MATCH_ANY)&&($option!=SPH_MATCH_PHRASE)) {
    	  	  $option = SPH_MATCH_ANY;
    	  	}
    	  }  
    	} else {
    	  $option = SPH_MATCH_PHRASE;
    	}
    	...
    	if(!empty($kword))
		{
               ...
               require_once (APPPATH.'config/sphinx'.EXT);
			    ...
			    $sphinx->SetFieldWeights(array ('name' => 20, 'short_content' => 10, 'content' => 5));
          ...
          $real_kword = $this->get_real_text($kword);
          
			    $resArray_sanalytics_news         = $sphinx->Query($real_kword,'i_sanalytics_news');	
			    $resArray_squestions              = $sphinx->Query($real_kword,'i_squestions');	
			    $resArray_spages                  = $sphinx->Query($real_kword,'i_spages');	
			    $resArray_spress_about            = $sphinx->Query($real_kword,'i_spress_about');	
			    $resArray_sanalytics_reviews      = $sphinx->Query($real_kword,'i_sanalytics_reviews');	
			    $resArray_sdownloads              = $sphinx->Query($real_kword,'i_sdownloads');
			    $resArray_snews                   = $sphinx->Query($real_kword,'i_snews');
			    $resArray_sconferencies           = $sphinx->Query($real_kword,'i_sconferencies');
                ...
                if($resArray_sanalytics_news===false or 
                   ...
                   echo "Query failed: " . $sphinx->GetLastError() . ".\n"; // выводим ошибку если произошла
                    	...
                    	$sanalytics_news = $this->formatSearchArr($resArray_sanalytics_news,0,$real_kword);
                        if(count($sanalytics_news)>0){
                            $items = $sanalytics_news;
                        }
						
                        $squestions     = $this->formatSearchArr($resArray_squestions,1,$real_kword);
						if(count($squestions)>0){
							if(count($items)>0) $items = array_merge($items,$squestions);  else $items = $squestions;
						}						
						
						$pages        = $this->formatSearchArr($resArray_spages,2,$real_kword);
                	    if(count($pages)>0){
							if(count($items)>0) $items = array_merge($items,$pages);  else $items = $pages;
						}						
						
						$press_about   = $this->formatSearchArr($resArray_spress_about,3,$real_kword);
						if(count($press_about)>0){
							if(count($items)>0) $items = array_merge($items,$press_about);  else $items = $press_about;
						} 						
						
						$reviews       = $this->formatSearchArr($resArray_sanalytics_reviews,4,$real_kword);
						if(count($reviews)>0){
							if(count($items)>0) $items = array_merge($items,$reviews);  else $items = $reviews;
						} 
						
						$downloads   = $this->formatSearchArr($resArray_sdownloads,5,$real_kword);
						if(count($downloads)>0){
							if(count($items)>0) $items = array_merge($items,$downloads);  else $items = $downloads;
						}
						$news   = $this->formatSearchArr($resArray_snews,6,$real_kword);
						if(count($news)>0){
							if(count($items)>0) $items = array_merge($items,$news);  else $items = $news;
						}
						$conferencies   = $this->formatSearchArr($resArray_sconferencies,7,$real_kword);
