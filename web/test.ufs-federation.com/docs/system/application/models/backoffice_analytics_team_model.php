<?php
class backoffice_analytics_team_model extends Model{

	function backoffice_analytics_team_model()
	{
		parent::Model();
        ...
        if(isset($_REQUEST['submit'])){
            ...
            $next_id=$this->global_model->Auto_inc($this->page_name);
              ...
              $ret['lang']=$this->site_lang;
                ...
                if(isset($_FILES['img'])){
                  $ret['img']=upload_pic($_FILES['img'],'','upload/analytical_team','img_'.$next_id.'_'.$this->site_lang.'_',true,'',$this->subdir); 
                  if(!empty($ret['img'])){
        ...
        if(isset($_REQUEST['submit'])){
            $ret=$this->global_model->adjustment_of_request_array($_REQUEST);	
              ...
              $ret['display']=isset($ret['display'])?implode(',',$ret['display']):'';
                ...
                if(isset($_FILES['img'])){
                  $ret['img']=upload_pic($_FILES['img'],$_REQUEST['img_old'],'upload/analytical_team','img_'.(int)$_REQUEST['id'].'_'.$this->site_lang.'_',true,'',$this->subdir); 
                  if(!empty($ret['img'])){
                      self::img_resize($ret['img']);
                  }
                }
    ...
    function sort()
    {
        return $this->global_model->item_sort($this->uri->segment(4),$this->uri->segment(5),$this->uri->segment(6),$this->page_name);  
    }
    ...
    { 
      $content['data']='';
        ...
        if($sql_r->num_rows()>0){	
            ...
            $vals='`t`.`id`, `t`.`name`, `t`.`display`, `t`.`sort_id`';
    ...
    {
        img_resize_new('upload/analytical_team/'.$img,'upload/analytical_team/small/'.$img,92,0,0xFFFFFF,100,$this->subdir);
    ...
    private function img_del($id)
    {
     $sql=$this->db->query('select `img` from `'.$this->page_name.'` where `id`="'.$id.'" and `lang`="'.$this->site_lang.'" limit 1;');
        if($sql->num_rows()>0){	
            $res=$sql->row(); 
            $dir=$_SERVER['DOCUMENT_ROOT'].'/upload/analytical_team/';
