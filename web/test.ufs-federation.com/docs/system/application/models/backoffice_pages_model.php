...
class backoffice_pages_model extends Model{
    ...
    function add()
    {
      ...
      if (isset($_REQUEST['submit'])) {
        ...
        $ret['main']=(isset($ret['main']) && ($ret['main']==1))?'1':'0';
        $ret['bubble_duration']=isset($ret['bubble_duration'])?$ret['bubble_duration']:NULL;
        $ret['bubble_count']=isset($ret['bubble_count'])?$ret['bubble_count']:NULL;
        ...
        if(isset($_REQUEST['submit'])){
            ...
            for($i=1;$i<=6;$i++){ 
             ...
             $ret[$promo]=isset($ret[$promo])?'yes':'no';
              ...
              if (isset($ret['contact_id']) && ($ret['contact_id']=="")) {
              	$ret['contact_id']=NULL;
              } 
              
              if (isset($ret['slider_link_type']) && ($ret['slider_link_type']=="")) {
              	$ret['slider_link_type']=NULL;
              }
              
              if (isset($ret['redirect_code']) && ($ret['redirect_code']=='')) {
              	$ret['redirect_code'] = NULL;
              }
      ...
      $content['data']='';
        ...
        if($sql_r->num_rows()>0){	
            ...
            $vals='`t`.`id`, `t`.`name`, `t`.`cat_id`, `t`.`sort_id`, `t`.`is_delete`, `t`.`timestamp`, `t`.`parent_id`, `t`.`sub_page_type`, `t`.`link_to_page`, 
                   t.bubble_duration, t.bubble_count, t.company ';
...
/************************************************************************************/
    ...
    private function get_parent_pages($cat_id,$id=0)
    {
       ...
       $sub_page_types = '';
       foreach($this->global_model->open_types as $s) {
         $s = '`sub_page_type`="'.$s.'" ';
         $sub_page_types.= (($sub_page_types=='')?$s:'or '.$s);
       }
       ...
       for($i=0;$i<count($ret);$i++) {
           ...
           $sql=$this->db->query('select `id`, `name` 
                                  ...
                                  and `parent_id`="0"
                                  '.(($sub_page_types!='')?'and ('.$sub_page_types.')':'').'
                                  and `link_to_page`=""
                                  and `lang`="'.$this->site_lang.'"
