<?php
class backoffice_popup_case_disclosure_of_information_menu_model extends Model{

	function backoffice_popup_case_disclosure_of_information_menu_model()
	{
		parent::Model();
	}
    ...
    function add($table,$content,$page_name)
    {
      $content['data']='';
        ...
        if(isset($_REQUEST['submit'])){
            ...
            $ret=$this->global_model->adjustment_of_request_array($_REQUEST);
                ...
                if(empty($error)){
                    ...
                    $ret['calendar_type']=$_REQUEST['calendar_type'];
                    if ($ret['calendar_type']=='') {
                    	$ret['calendar_type']=null;
                    }
        ...
        $content['calendar_types']=self::get_calendar_types(null,' selected="selected"');

       return $this->load->view('backoffice_'.$this->page_name.'_edit_disclosure_of_information_menu',$content,true);
    }
      ...
      $content['data']='';
        ...
        if(isset($_REQUEST['submit'])){
            ...
            if($sql->num_rows>0){
                ...
                $content['calendar_types']=self::get_calendar_types($res->calendar_type,' selected="selected"');
            }else{
                $set_id=0;
    ...
    function del($table,$content,$page_name)
    {
        $sql=$this->db->query('select `id` from `'.$table.'` where `parent_id`="'.(int)$this->uri->segment(5).'" or `id`="'.(int)$this->uri->segment(5).'" and `lang`="'.$this->site_lang.'" limit 1;');
            if($sql->num_rows()>0){	
                ...
                $sql_f=$this->db->query('select `_file` from `disclosure_of_information` where `cat_id` in ('.(int)$this->uri->segment(5).','.$res->id.') and `lang`="'.$this->site_lang.'";');
                    if($sql_f->num_rows()>0){	
                        $res_f=$sql_f->result();
                ...
                $this->db->query('delete from `disclosure_of_information` where `cat_id` in ('.(int)$this->uri->segment(5).','.$res->id.') and `lang`="'.$this->site_lang.'";');
    ...
    {         
        ...
        return $this->load->view('backoffice_'.$this->page_name.'_view_disclosure_of_information_menu',$content,true);
    }
...
######################################################################################################################
    ...
    private function get_calendar_types($calendar_type=null,$sel='sel')
    {
    	...
    	for($i=0;$i<count($res);$i++) {
    		...
    		if (!is_null($calendar_type)) {
    			$res[$i]->sel=($res[$i]->id==$calendar_type)?$sel:'';
    		} else {
    			$res[$i]->sel='';
    		}
