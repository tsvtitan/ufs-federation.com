...
class backoffice_commodities_model extends Model{

	function backoffice_commodities_model()
	{
		parent::Model();
    ...
    function export_xls()
    {
        $vals='`t`.`name`, `t`.`ticker`, `t`.`sector`, `t`.`price_current`, 
               `t`.`price_fair`, `t`.`potential`, `t`.`recommendation`';
        $where='';
        $limit=''; 
        ...
        header("Content-Disposition: attachment;filename=commodities-".date("d-m-Y")."-export.xls");
    ...
    function recalc() {
      ...
      if ($service->connect()) {
        ...
        $r = $service->startJob('www/Commodities');
    ...
    { 
      $content['data']='';
        ...
        if($sql_r->num_rows()>0){	
            ...
            $vals='`t`.`id`, `t`.`name`, `t`.`ticker`, `t`.`sector`, `t`.`price_current`, 
                   `t`.`price_fair`, `t`.`potential`, `t`.`recommendation`';
            $sql=$this->db->query(self::view_sql_str($vals,$where,$limit));       
            ...
            for($r=$xls->sheets[0]['numRows'];$r>=4;$r--)
            {
               $c=1;
                ...
                if (trim($name)<>'') {
                  ...
                  $arr[$r]['text']['price_current']=$xls->sheets[0]['cells'][$r][++$c];
