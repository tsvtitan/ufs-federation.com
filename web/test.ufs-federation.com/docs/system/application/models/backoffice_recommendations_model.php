...
class backoffice_recommendations_model extends Model{

	function backoffice_recommendations_model()
	{
		parent::Model();
    ...
    function export_xls()
    {
        $vals='`t`.`name`, `t`.`isin`, `t`.`ticker`, `t`.`price_currency`, 
    ...
    function recalc() {
      ...
      if ($service->connect()) {
        ...
        $r = $service->startJob('www/Recommendations');
    ...
    { 
      $content['data']='';
        ...
        if($sql_r->num_rows()>0){	
            ...
            $vals='`t`.`id`, `t`.`name`, `t`.`isin`, `t`.`ticker`, `t`.`price_currency`, 
                   `t`.`price_fair`, `t`.`potential`, `t`.`recommendation`';
