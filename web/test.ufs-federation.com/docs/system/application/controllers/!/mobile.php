...
class Mobile extends ControllerEx {
  ...
  private function get_device_type_id() {
    ...
    $ret = null;
    $params = $this->get_params(array('manufacturer'=>'madeBy','model'=>'deviceModel','os'));
  ...
  private function tran_begin($make_session=false) {
    ...
    if (!isset($msid) && $make_session) {
      ...
      if ($lang) {
        $s = new stdClass();
        $s->mobile_session_id = $this->provider->get_unique_id();
    ...
    if (isset($msid)) {
      ...
      if ($session) {
        ...
        if (!is_null($session->locked)) {
          $ret->error->message = $session->lock_reason;
    ...
    if ($ret) {
      $c = new stdClass();
      $c->mobile_comm_id = $this->provider->get_unique_id();
    ...
    if ($this->is_work()) {
      ...
      if ($tran) {
        try {
          ...
          $this->provider->insert('test_del',array('id'=>1,'name'));
  ...
  private function get_file_url($tran,$location,$name=null,$extension=null,$content_type=null,&$file_size=false,$prev_delete=false) {
    ...
    if (isset($tran) && isset($tran->session)) {
      ...
      if (!$f) {
        ...
        $f = new stdClass();
        $f->mobile_file_id = $this->provider->get_unique_id();
