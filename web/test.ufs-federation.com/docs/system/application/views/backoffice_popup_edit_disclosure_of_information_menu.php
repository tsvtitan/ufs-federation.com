  ...
  <tr>
	...
	<form action="<? echo($this->uri->uri_string); ?>" method="post">
		<table border="0" cellspacing="0" cellpadding="0" align="center" class="form_content">
		  ...
		  <tr>
		  	<td class="item">
				<select name="int[parent_id]" class="select">
					<option value="0">&nbsp;&nbsp;-&nbsp;<? echo($this->lang->line('admin_tpl_main_category')); ?>&nbsp;-</option>
					<? foreach($cat_menu as $item){ ?>
					  <option value="<? echo($item->id); ?>"<? echo($item->select); ?>><? echo($item->name); ?></option>
					<? } ?>
				</select>
			</td>
		  </tr>
		  ...
		  <tr>
			<td class="header_title"><h5><? echo($this->lang->line('admin_tpl_calendar_type')); ?></h5></td>
		  </tr>
		  <tr>
		  	<td class="item">
				<select name="calendar_type" class="select">
					<option><? echo(""); ?></option>
					<? foreach($calendar_types as $item){ ?>
					  <option value="<? echo($item->id); ?>"<? echo($item->sel); ?>><? echo($item->name); ?></option>
					<? } ?>
				</select>
			</td>
		   </tr>
