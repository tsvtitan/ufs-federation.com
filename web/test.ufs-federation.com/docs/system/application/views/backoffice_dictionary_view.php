  ...
  <tr>
	...
	<td valign="top" align="center">
		<table border="0" cellspacing="0" cellpadding="0" align="center" class="form_content">
          <? if($data){ ?>
		  <tr>
            <td class="header_title center"><h5><? echo($this->lang->line('admin_dictionary_ru')); ?></h5></td>
            <td class="header_title center"><h5><? echo($this->lang->line('admin_dictionary_en')); ?></h5></td>
            <td class="header_title center"><h5><? echo($this->lang->line('admin_dictionary_de')); ?></h5></td>
            <td class="header_title center"><h5><? echo($this->lang->line('admin_dictionary_it')); ?></h5></td>
            <td class="header_title center"><h5><? echo($this->lang->line('admin_dictionary_fr')); ?></h5></td>
            <td class="header_title center"><h5><? echo($this->lang->line('admin_dictionary_el')); ?></h5></td>
            <td class="header_title center"><h5><? echo($this->lang->line('admin_dictionary_cn')); ?></h5></td>
			<td class="header_title center"></td>
          </tr>
		  <? foreach($data as $item){ ?>
          <tr<? echo($item->css_class); ?>>
            <td class="item center" onclick="javascript:prompt('Скопируйте этот код и вставьте в нужном месте темплейта.','&lt;? echo(dictionary(\'<? echo(mysql_string($item->ru)); ?>\')); ?&gt;');">
                <? echo($item->ru); ?>
            </td>
            <td class="item center"><? echo($item->en); ?></td>
            <td class="item center"><? echo($item->de); ?></td>
            <td class="item center"><? echo($item->it); ?></td>
            <td class="item center"><? echo($item->fr); ?></td>
            <td class="item center"><? echo($item->el); ?></td>
            <td class="item center"><? echo($item->cn); ?></td>
			<td class="item">
