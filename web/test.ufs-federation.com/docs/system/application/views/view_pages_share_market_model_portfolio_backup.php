        ...
        <div id="content_s">                    
          ...
          <div id="content_tab_<? echo($i); ?>" class="content-tab">
           ...
           <? if(count($data->graph[$i])>1){ ?>
                ...
                <div class="linechart">	 
                    <div id="graph<? echo(($i>1)?$i:''); ?>">Loading graph...</div>
                </div> 
                ...
                var <? echo('myData'.$i); ?> = new Array(<? 
                                                          $x=0;
                                                            foreach($data->graph[$i] as $item){
                                                             ...
                                                             echo('['.$item->timestamp.','.$item->transaction_amount.']');
                                                            } 
                                                         ?>);
                ...
                <? 
                   foreach($data->graph[$i] as $item){
                     echo('myChart'.$i.'.setTooltip(['.$item->timestamp.', \''.$item->date.' - '.$item->transaction_amount.'\']);'."\n");
                   } 
                ?>
