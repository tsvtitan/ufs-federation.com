    ...
    <td valign="top" align="center" class="left_sidebar">
        <table border="0" cellspacing="0" cellpadding="0" align="center" class="left_content">
          ...
          <tr>
            <td class="item">
                <ul class="sub_menu">
                    <li><a href="<? echo($this->phpself.$this->page_name.'/add/'.$this->role_id); ?>"><? echo($this->lang->line('admin_tpl_add')); ?></a></li>
                    <li><a href="javascript:void(0)"  onclick="javascript:window.open('<? echo($this->phpself); ?>popup/roles','popup','width=550,height=600,left=50,top=50,scrollbars=yes');"><? echo($this->lang->line('admin_roles_editor')); ?></a></li>
                </ul>
            </td>
          </tr>
          <tr>
                <td class="header_title"><h5><? echo($this->lang->line('admin_roles')); ?></h5></td>
          </tr>
          <tr>
            <td class="item">
                <ul class="nav_menu">
                    <? foreach($roles as $val){ ?>
                      <li>
                          <a<? echo($val->sel); ?> href="<? echo($this->phpself.$this->page_name); ?>/role/<? echo($val->id); ?>"><? echo($val->role); ?></a>
                      </li>
                    <? } ?>
                </ul>
            </td>
		...
		<table border="0" cellspacing="0" cellpadding="0" align="center" class="form_content">
		  ...
		  <? foreach($data as $item){ ?>
              <tr<? echo($item->css_class); ?>>
                <td class="item" width="100%"><? echo($item->path); ?></td>
                <td class="item center"><? echo($item->access); ?></td>
                <td class="item">
                    <table border="0" cellspacing="0" cellpadding="0">
                        <tr>
                            <td><a href="<? echo($this->phpself.$this->page_name.'/edit/'.$item->role_id.'/'.$item->id); ?>" title="<? echo($this->lang->line('admin_tpl_edit')); ?>"><img src="<? echo($this->base_url); ?>images/pencil.png" alt="<? echo($this->lang->line('admin_tpl_edit')); ?>" width="16" height="16" /></a></td>
                            <td><a href="<? echo($this->phpself.$this->page_name.'/del/'.$item->role_id.'/'.$item->id); ?>" title="<? echo($this->lang->line('admin_tpl_del')); ?>" onclick="return confirm('<? echo($this->lang->line('admin_tpl_del')); ?>?'); false;"><img src="/images/delete.png" alt="<? echo($this->lang->line('admin_tpl_del')); ?>" width="16" height="16" /></a></td>
                        </tr>
                    </table>
                </td>
              </tr>
