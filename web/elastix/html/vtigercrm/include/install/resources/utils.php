...
class Migration_Utils {
	...
	static function getInstallableOptionalModules($old_version) {
		$optionalModules = Common_Install_Wizard_Utils::getInstallableModulesFromPackages();
		...
		$oldVersion = str_replace(array('.', ' '), array('', ''),$old_version);
	...
	function migrate($migrationInfo){
		...
		if(!isset($source_version) || empty($source_version)) {
			...
			echo "No existe la fuente anterior, no se puede proceder";
			exit;
		}
		...
		for($patch_count=0;$patch_count<count($temp);$patch_count++) {
			...
			$filename = $migrationInfo['root_directory']."modules/Migration/DBChanges/".$temp[$patch_count]."_to_".$temp[$patch_count+1].".php";
			//$empty_tag = "<tr><td colspan='2'>&nbsp;</td></tr>";
			//$start_tag = "<tr><td colspan='2'><b><font color='red'>&nbsp;";
			//$end_tag = "</font></b></td></tr>";
	
			if(is_file($filename)) {
				//echo $temp[$patch_count]." ==> ".$temp[$patch_count+1]. " " .$installationStrings['LBL_DATABASE_CHANGES'] ." -- ". $installationStrings['LBL_STARTS'] .".".$end_tag;
